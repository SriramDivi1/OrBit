...
export function Header({ onMenuClick, onSearch, onSignOut, onNavigate, user }: HeaderProps) {
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-100 px-4 lg:px-8 flex items-center justify-between sticky top-0 z-20 shadow-sm">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button 
              className="relative p-2 text-gray-500 hover:bg-gray-50 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#FF971D]/50"
            >
              <Bell className="w-5 h-5" />
              {unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-[#F05252] rounded-full border-2 border-white ring-1 ring-white"></span>
              )}
            </button>
          </PopoverTrigger>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button 
              className="flex items-center gap-3 pl-1 pr-2 py-1 hover:bg-gray-50 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-[#FF971D]/50"
            >
              <img 
              ...
              <div className="hidden md:block text-left">
                <p className="text-sm font-semibold text-gray-900">{user?.name || "Tom Cook"}</p>
